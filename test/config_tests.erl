...
-export([handle_config_change/5, handle_config_stop/3]).
...
handle_config_change(Section, Key, Value, Persist, State) ->
    ...
    {ok, State}.
handle_config_stop(Self, Reason, State) ->
...
config_listener_behaviour_tests() ->
    {
        "Test config_listener behaviour",
        {
            ...
            [
                ...
                fun should_pass_correct_state_to_handle_config_stop/1,
                fun should_pass_subscriber_pid_to_handle_config_stop/1,
...
should_handle_value_change({Pid, _, _}) ->
    ?_test(begin
        ok = config:set("httpd", "port", "80", false),
        ?assertMatch({{"httpd", "port", "80", false}, _}, wait_reply(Pid))
    end).
should_pass_correct_state_to_handle_config_change({Pid, _, _}) ->
    ?_test(begin
        ok = config:set("httpd", "port", "80", false),
...
should_pass_correct_state_to_handle_config_stop({Pid, _, _}) ->
    ?_test(begin
        %% prepare some state
        ok = config:set("httpd", "port", "80", false),
        ?assertMatch({_, {Pid, _, []}}, wait_reply(Pid)),
...
should_pass_subscriber_pid_to_handle_config_stop({Pid, SubscriberPid, _}) ->
    ?_test(begin
        ok = config:set("remove_handler", "any", "any", false),
        Reply = wait_reply(Pid),
        ?assertMatch({stop, _, remove_handler, _}, Reply),
...
should_not_call_handle_config_after_related_process_death({Pid, _, _}) ->
    ?_test(begin
        ok = config:set("remove_handler", "any", "any", false),
        Reply = wait_reply(Pid),
        ?assertMatch({stop, _, remove_handler, _}, Reply),
...
should_remove_handler_when_requested({Pid, _, _}) ->
    ?_test(begin
